...
Rotation itself is computed locally by each validator from the hardcoded allowlist
...
from leoma.infra.db.stores import SampleStore, SamplingStateStore
...
async def ordered_validators() -> List[str]:
    """Deterministic order of the hardcoded validator allowlist (rotation order = sorted hotkeys)."""
    return sorted(set(VALIDATOR_ALLOWLIST))
